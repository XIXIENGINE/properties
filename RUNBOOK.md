...
| 브리핑이 안 오고 "앱이 데이터베이스에 연결하지 못했습니다" | Supabase 일시정지. 대시보드에서 Restore 후 Actions 재실행 |
| "앱 인증에 실패했습니다" | `NYANGDOL_APP_PASSWORD` 가 앱의 `APP_PASSWORD` 와 다름 |
| "앱 CSV 형식이 예상과 다릅니다" | 앱의 내보내기 컬럼이 바뀜. `app_source.py` 의 변환부를 맞춰야 함 |
| 앱이 오래 내려가 급히 필요할 때 | Actions 실행 시 `BRIEF_SOURCE=sheet` (저장소 변수)로 옛 시트 경로 사용 |

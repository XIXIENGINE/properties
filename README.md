...
- **데이터 출처**: 냥돌폴리오 앱 (`NYANGDOL_APP_URL`)의 보유현황 CSV.
  앱이 거래를 접어 보유수량·평단을 내고 실시간 시세·환율까지 반영하므로,
  브리핑은 **다시 계산하지 않고 그 값을 그대로 쓴다.** 계산 경로가 둘이면
  같은 자산이 화면과 브리핑에서 다른 금액으로 보인다.
- **집계 범위**: 앱에 등록된 가계 전체(냥이·돌이) + 계좌 현금잔액.
  보유자별 비중·수익률을 별도 섹션으로 표시한다.
- **비상용 출처**: 앱이 오래 내려가면 `--source sheet` 로 구글 스프레드시트
  `금융자산현황표` 를 직접 파싱하는 옛 경로를 쓸 수 있다(시세 자체 조회 포함).
...
1. `generate.py` 가 냥돌폴리오 앱의 `/api/export/holdings` CSV를 받아 파싱
   - 앱 비밀번호(`NYANGDOL_APP_PASSWORD`)로 만든 인증 쿠키를 붙여 요청
   - 평가액·매입액·수익률이 이미 계산된 값이라 **여기서 시세를 다시 조회하지 않는다**
   - 받지 못하면(일시정지·인증 실패 등) 원인을 Slack으로 알리고 종료한다
...
### 실시간 시세 자동 계산 (LIVE_PRICES) — 시트 소스 전용

> 기본 출처가 앱으로 바뀐 뒤로 이 경로는 `--source sheet` 일 때만 쓰입니다.
> 앱 소스에서는 앱이 이미 시세·환율을 반영하므로 `LIVE_PRICES` 를 무시합니다.

시트 소스에서는 `LIVE_PRICES=1` 로 시트의 **수량 열**에 현재가를 곱해
평가금액을 자동 산출합니다.
...
| `generate.py` | 데이터 수집(앱/시트)·Slack 발송·스냅샷 저장 (Actions용) |
| `app_source.py` | 냥돌폴리오 앱 CSV 다운로드·파싱 (기본 출처) |
| `prices.py` | 실시간 시세 조회 — **시트 소스에서만** 사용 |
...
- 앱: 냥돌폴리오 (`XIXIENGINE/nyangdol-folio`) — 시크릿 `NYANGDOL_APP_URL`
- 스프레드시트 ID(비상용): `1MLuYEdJUUuhQ6V3hp7WSmsIFeFRo9LIHL23_wcdezFw`
